...
const { hoy, rangoDelDia } = require('./fechas');
...
const tiendas = {
  ...
  conBeneficio: (id) => una(
    `SELECT t.id, t.nombre, t.rubro, t.estado,
       ...
       LEFT JOIN beneficios b
              ON b.tienda_id = t.id
             AND b.vigente_desde <= ?
             AND (b.vigente_hasta IS NULL OR b.vigente_hasta >= ?)
      WHERE t.id = ?
  ...
  directorio: () => consultar(
    `SELECT t.id, t.nombre, t.rubro, t.direccion, t.distrito, t.lat, t.lng,
       ...
       FROM tiendas t
       LEFT JOIN beneficios b
              ON b.tienda_id = t.id AND (b.vigente_hasta IS NULL OR b.vigente_hasta >= ?)
      WHERE t.estado = 'activa'
      ORDER BY t.rubro, t.nombre`, [hoy()])
};
...
// -------------------------------------------------------------------- canjes
const canjes = {
  // El rango se calcula en la zona del programa: con DATE(fecha)=CURDATE()
  // el tope se reiniciaria segun la zona del servidor MySQL, no la de Lima.
  usosHoy: async (tarjetaId, tiendaId) => {
    const { inicio, fin } = rangoDelDia();
    const f = await una(
      `SELECT COUNT(*) AS n FROM canjes
        WHERE tarjeta_id = ? AND tienda_id = ? AND fecha >= ? AND fecha < ?`,
      [tarjetaId, tiendaId, inicio, fin]);
    return Number(f.n);
  },
...
const carnet = {
  porToken: (token) => una(
    `SELECT t.token, t.estado, t.vence, t.emitida_en, t.cliente_id,
            c.nombre, c.doc
       FROM tarjetas t LEFT JOIN clientes c ON c.id = t.cliente_id
      WHERE t.token = ?`, [token]),

  /** Historial del titular. Solo tras identificarse. */
  historial: (clienteId, limite = 30) => consultar(
    `SELECT k.fecha, k.monto, t.nombre AS tienda, b.descripcion AS beneficio
       FROM canjes k JOIN tiendas t ON t.id = k.tienda_id
       JOIN beneficios b ON b.id = k.beneficio_id
      WHERE k.cliente_id = ?
      ORDER BY k.fecha DESC LIMIT ${Number(limite) | 0}`, [clienteId]),

  /** Autoactivacion: el titular registra su propia tarjeta en blanco. */
  activar: (token, cliente) => enTransaccion(async (cx) => {
    const [[tarjeta]] = await cx.execute(
      "SELECT id FROM tarjetas WHERE token = ? AND estado = 'en_blanco'", [token]);
    if (!tarjeta) throw new Error('Esa tarjeta no existe o ya fue activada');

    const [[existente]] = await cx.execute('SELECT id FROM clientes WHERE doc = ?', [cliente.doc]);
    let clienteId = existente?.id;
    if (!clienteId) {
      const [r] = await cx.execute(
        `INSERT INTO clientes (nombre, doc, celular, correo, consentimiento_en)
         VALUES (?, ?, ?, ?, NOW())`,
        [cliente.nombre, cliente.doc, cliente.celular || null, cliente.correo || null]);
      clienteId = r.insertId;
    }
    const vence = hoy(new Date(Date.now() + 365 * 864e5));
    await cx.execute(
      `UPDATE tarjetas SET cliente_id = ?, estado = 'activa', vence = ?, emitida_en = NOW()
        WHERE id = ?`, [clienteId, vence, tarjeta.id]);
    return { clienteId, vence };
  })
};

module.exports = { usuarios, tiendas, tarjetas, canjes, validaciones, admin, carnet };
